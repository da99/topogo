...
function log() {
  if (process.env.IS_DEV && !process.env.IS_TEST)
    return console['log'].apply(console, arguments);
}

function fin(flow, func) {
  return {
    finish: function () {
      if (arguments.length !== 1)
        return flow.finish.apply(flow, arguments);
...
T.run_and_return_1 = function (t, q, vars, flow) {
  River.new(flow)
  .job(function (j) {
    T.run(t, q, vars, j);
  })
  .job(function (j, rows) {
    if (rows.length > 1) {
      log(q);
      return j.finish('error', 'Too many results.');
    }
    if (rows.length < 1) {
      log(q)
      return j.finish('error', 'No row returned.');
    }
    j.finish(rows[0]);
  })
  .run();
...
T.run_and_return_at_most_1 = function (t, q, vars, flow) {
  River.new(flow)
  .job(function (j) {
    T.run(t, q, vars, j);
  })
...
T.run = function (topogo, q, vars, flow) {
  ...
  if (q.indexOf('$')) {
    q = q
    .replace(/\$created_at/, 'created_at $now_tz NOT NULL')
  ...
  _.each(vars, function (v) {
    if (_.isObject(v) && v.constructor === Date)
      clean_vars.push(v);
    else
      clean_vars.push(e_e_e(v));
  });

  pool[topogo.pool_key].query(q, clean_vars, function (err, result) {
    if (err) {
...
T.prototype.run_and_return_1 = function (sql, vals, flow) {
  return T.run_and_return_1(this, sql, vals, flow);
};

T.prototype.run_and_return_at_most_1 = function (sql, vals, flow) {
  return T.run_and_return_at_most_1(this, sql, vals, flow);
};
...
T.prototype.create = function (data, flow) {
  ...
  T.run(me, sql, vals, fin(flow, function (j, result) {
    j.finish(result[0]);
  }));
};
...
T.prototype.read_one = function (doc, flow) {
  ...
  T.run(me, sql, vals, fin(flow, function (j, last) {
    j.finish(last[0]);
  }));
};
...
T.prototype.update_where_set = function (where, set, flow) {
...
T.prototype.update_one = function (where, set, flow) {
  this.update_where_set(where, set, fin(flow, function (j, result) {
    if (_.isArray(result))
      j.finish(result[0]);
    else
      j.finish(result);
...
T.prototype.update_and_stamp = function (where, set, flow) {
  set['updated_at'] = '$now';
  return this.update_where_set(where, set, flow);
};
...
T.prototype.untrash = function (id, j) {
  var me = this;
  me.update_where_set(id, {trashed_at: null}, j);
};

T.prototype.trash = function (id, flow) {
  var me = this;
  me.run("UPDATE " + me.quoted_table +  " SET trashed_at = $now WHERE id = $1 RETURNING id, trashed_at;", 
         [id],
         fin(flow, function (j, last) {
           j.finish(last[0]);
         })
        );
};
...
T.prototype.trash_list = function (data, flow) {
  ...
  me.update_where_set(data, {trashed_at: '$now'}, flow);
};
